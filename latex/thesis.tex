...
Furthermore, two modes of measurement of MUAP arise: \textit{unipolar} measurements referred to a common potential and \textit{bipolar} measurements where consecutive electrodes' potentials are taken into differential amplifiers, as illustrated in figure \ref{fig:unibipolar}.
...
As a practical result, during low effort motion where only one or a handful of motor units are engaged, the MUAP is very easily identifiable in the measured waveform. On the other hand, high effort contractions will manifest a superposition of uncorrelated MUAPs which are difficult to separate from just sEMG measurements. \todo{FIGURE from own measurements}
...
Anecdotally, this is the board that sparked the idea for this project, being one of the experimental boards given to TUCN students. The Quanser QNET Myoelectric trainer board is a closed-source, one channel addon board for the National Instruments Elvis II. It provides a fixed amplification stage of ?gain? powered by a battery to remove power supply noise, and further uses the ELVIS' oscilloscope channel for analog to digital conversion. 

Depends on NI Elvis; one channel; not open \todo{more :)}

\subsection*{BioWare EXG Pills}

As one of the cheapest options, the EXG Pill is an open source instrumentation amplifier board meant to aid in biopotential data acquisition by connecting it to another device with an ADC, typically an Arduino board. It is available for order on the creator's online shop, but being an open hardware project allows anyone to manufacture their own, to specification.
...
The superposition of electrical signals measurable by biopotential DAQs contains a complicated mix of noise coming from various physiological and other external factors, besides the signal of interest. Additionally, the measurement circuitry compounds noise at every step. Modeling some of the main noise components of the entirawe signal chain is essential for its design and proper specification.
...
Later used in both the preamplifier and Driven Right Leg circuits, the common collector amplifier, illustrated in figure ?? is a minimal and cheap circuit for providing a low impedance, high drive strength "mirror" of an incoming signal with potentially higher impedance.
...
The chosen AD4114 ADC has $N=24$ bits of resolution with one LSB equating to $\text{LSB} = V_{REF} / (2^N \cdot 0.1)$. Using the internal $2.5\V$ reference, $\text{LSB} \approx 1.49\uV$. The noise level varies depending on the sample rate. At the wanted $f_s=1007\Hz$ and maximum $f_s=6211\Hz$, the ADC noise is specified to be around $\overline{V_{na,1007}^2}=20\uV$ and $\overline{V_{na,6211}^2}=74\uV$ RMS respectively. Without an amplification stage, this would result in the following SNRs (assuming a $1\mV$ signal):
...
Gain & Approx. full scale & $\text{SNR}$ \\ \hline
1    & $5\V$      & $34\dB$ \\
2    & $2.5\V$    &  $40\dB$ \\
5    & $1\V$      &  $48\dB$ \\
10   & $500\mV$   &  $54\dB$ \\
...
Using an active element (the instrumentation amplifier) also gives us the opportunity to do some hardware filtering. To combat high frequency noise, we choose to add a first order RC low-pass filter with a cutoff of $f_c \approx 500\Hz$. Our implementation uses $R=330\ohm$ and $C=1\uF$ for a theoretical $3\dB$ cutoff frequency of:

$$f_c = \frac{1}{2\pi \cdot 330\ohm \cdot 1\uF} = 482.28\Hz$$

To encode the bipolar differential measurement as a unipolar signal, we bias the amplifier's output voltage using the AD8226's $V_{REF}$ input. As specified, this pin must be driven with less than $2\ohm$ impedance, so just a simple resistor divider wouldn't suffice. We use a common-collector voltage buffer after a $10\kohm - 10\kohm$ divider to amplify the drive strength, biasing the output to approximately $V_{REF} \approx 5\V / 2 - 0.7\V = 1.8V$.

To help with transient power supply fluctuations, the instrumentation amplifier's positive supply is also decoupled with a $0.1\uF$ capacitor.
...
Secondly, we test the frequency response using the Network analyzer feature of the Scopy software\cite{scopy}. A $10\Hz - 10\kHz$ sine sweep is generated, plugged into the amplifier, and the resulting output is measured, determining both the magnitude and phase relation, which are plotted in figure \ref{fig:preampbode}. Both the measured magnitude and phase characteristics match the ideal ones well enough, especially given the $10\%$ tolerances of the passives, successfully validating the performance of this part of the signal chain.


\begin{thesisfigure}{Circuit diagram and photo of built amplifier board}{ampboard}

\includegraphics[width=0.6\linewidth]{ampboard_schematic.png}\\
\includegraphics[width=0.6\linewidth]{ampboard_photo.jpg}

\end{thesisfigure}

\begin{thesisfigure}{Preamplifier board frequency response, measured and ideal $500\Hz$ first order LPF}{preampbode}
\begin{tikzpicture}
